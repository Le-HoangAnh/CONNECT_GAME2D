...
namespace Connect.Core
{
    public class GameplayManager : MonoBehaviour
    {
        ...
        [SerializeField] private SpriteRenderer _boardPrefabs, _bgCellPrefabs;

        private void SpawnBoard()
        {
            int currentLevelSize = GameManager.Instance.CurrentStage + 4;

            var board = Instantiate(_boardPrefabs,
                new Vector3(currentLevelSize / 2f, currentLevelSize / 2f, 0f), Quaternion.identity);
            ...
            for (int i = 0; i < currentLevelSize; i++)
            {
                ...
                {
                    Instantiate(_bgCellPrefabs, new Vector3(i + 0.5f, j + 0.5f, 0f), Quaternion.identity);
                }
            }
        ...
        private void SpawnNodes()
        {
            ...
            for (int i = 0; i < currentLevelSize; i++)
            {
                for (int j = 0; j < currentLevelSize; j++)
                {
                    ...
                    spawnedNode = Instantiate(_nodePrefabs, spawnPos, Quaternion.identity);
                    spawnedNode.Init();
